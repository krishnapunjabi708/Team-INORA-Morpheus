...
def get_ph(region, start, end):
    ...
    sal = col.expression('(B11-B8)/(B11+B8+1e-6)',{
        'B11':col.select('B11'),'B8':col.select('B8')
    })
    ph_img = col.expression('7.1+0.15*B2-0.32*B11+1.2*br-0.7*sa',{
        'B2':col.select('B2'),'B11':col.select('B11'),'br':br,'sa':sal
    }).rename('ph')
    val = ph_img.reduceRegion(ee.Reducer.mean(), region, scale=10, maxPixels=1e13).get('ph')
...
def get_salinity(region, start, end):
    ...
    val = ndsi.reduceRegion(ee.Reducer.mean(), region, scale=10, maxPixels=1e13).get('ndsi')
    return float(val.getInfo()) if val else None
...
def get_organic_carbon(region, start, end):
    ...
    val = oc.reduceRegion(ee.Reducer.mean(), region, scale=10, maxPixels=1e13).get('oc')
    return float(val.getInfo()) if val else None
...
def estimate_cec(region, start, end, intercept, slope_clay, slope_om):
    col = (ee.ImageCollection('COPERNICUS/S2_SR_HARMONIZED')
           ...
           .median().multiply(0.0001).clip(region))
    ...
    clay_m = clay.reduceRegion(ee.Reducer.mean(), region, scale=20, maxPixels=1e13).get('clay')
    om_m = om.reduceRegion(ee.Reducer.mean(), region, scale=20, maxPixels=1e13).get('om')
...
def get_lst(region, start, end):
    """
    Fetch Land Surface Temperature (LST) from MODIS/061/MOD11A2 with buffering,
    date expansion, and robust reduction.
    """
    def fetch(r, sd, ed):
    ...
    # Compute median and convert to °C
    img = (coll
           .median()
           .multiply(0.02)
           .subtract(273.15)
...
if region:
    ...
    sm, sm_d = get_smap_moisture(lat, lon)
    
    st.write(f"**SMAP Moisture ({sm_d}):** {sm:.3f}" if sm else "**SMAP Moisture:** N/A")
