...
def initialize_ee():
    ...
    try:
        credentials_base64 = os.getenv("SERVICE_ACCOUNT_JSON")
...
def make_vegetation_chart(ndvi, evi, fvc, *args):
    try:
        ...
        for i, (bar, val) in enumerate(zip(bars, values)):
            ...
            status = "High" if val > high else "Low" if val < low else "Good"
...
def generate_report(params, location, date_range):
    try:
        ...
        for chart, img in [("Nutrient Levels", nutrient_chart), ("Vegetation Indices", vegetation_chart), ("Soil Properties", properties_chart)]:
            if img:
                elements.append(Paragraph(f"{chart}:", body))
                elements.append(Image(img, width=12*cm, height=6*cm))
...
class PolygonRequest(BaseModel):
    ...
    def check_polygon(cls, v):
        if len(v) < 4:
            raise ValueError('Polygon must have at least 3 points and be closed (first and last points identical)')
        if v[0] != v[-1]:
        ...
        for point in v:
            ...
            if not (-180 <= lon <= 180) or not (-90 <= lat <= 90):
                raise ValueError('Invalid coordinates: longitude must be [-180, 180], latitude [-90, 90]')
        return v

def process_data(region, start_date, end_date, cec_intercept, cec_slope_clay, cec_slope_om):
...
def generate_report_endpoint(request: PolygonRequest):
    try:
        # Extract polygon
        polygon = request.polygon
        
        # Set dates: end_date is today, start_date is 16 days prior
        end_date = date.today()
        ...
        # Process data
        params = process_data(region, start_date, end_date, cec_intercept, cec_slope_clay, cec_slope_om)
