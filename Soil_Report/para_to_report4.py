...
MODEL = "models/gemini-1.5-flash"  # Use 'gemini-2.0-flash' if available in your plan
LOGO_PATH = "LOGO.jpg"  
...
IDEAL_RANGES = {
    ...
    "EVI":            (0.2, 0.8),
    "FVC":            (0.3, 0.8),
    "NDWI":           (0, 0.5),      # Updated to 0-0.5
    "Nitrogen":       (280, 450),    # mg/kg
    ...
    try:
        ...
        for days in range(5, 31, 5):
            ...
            if coll.size().getInfo() > 0:
                ...
                return coll.median().multiply(0.0001)
        logging.warning("No Sentinel-2 data available for the specified range.")
        return None
    except Exception as e:
...
def get_lst(region, start, end):
    ...
    logging.info(f"Fetching MODIS LST from {start} to {end}")
    start_str = start.strftime("%Y-%m-%d")
    end_str = end.strftime("%Y-%m-%d")
    try:
        coll = (
        ...
        if coll.size().getInfo() == 0:
            logging.warning("No LST images in the past month.")
            return None
...
def generate_interpretation(param, value):
    ...
    if min_val is None and max_val is not None:
        ...
        return f"Optimal; within ideal range ({range_text})." if value <= max_val else f"High; above ideal range (> {max_val}). Consider drainage improvements."
    elif max_val is None and min_val is not None:
        ...
        if min_val is not None and max_val is not None:
            if min_val <= value <= max_val:
                ...
                return f"Low; below ideal range (< {min_val}). Consider supplementation."
            elif value > max_val:
                if param in ["Phosphorus", "Potassium"] and value <= max_val * 1.5:
                    return f"Slightly high; acceptable for some soils (> {max_val}). Monitor levels."
                return f"High; above ideal range (> {max_val}). Assess runoff risks."
        return f"Interpretation unavailable for {param}."
...
def make_nutrient_chart(n_val, p_val, k_val):
    try:
        ...
        for nutrient, bar in zip(nutrients, bars):
            min_val, max_val = IDEAL_RANGES[nutrient]
            plt.axhspan(min_val, max_val, color='green', alpha=0.1)
...
def make_vegetation_chart(ndvi, evi, fvc, ndwi):
    try:
        indices = ["NDVI", "EVI", "FVC", "NDWI"]
        values = [ndvi or 0, evi or 0, fvc or 0, ndwi or 0]
        colors = [get_color_for_value(index, value) for index, value in zip(indices, values)]
        plt.figure(figsize=(6, 4))
        bars = plt.bar(indices, values, color=colors, alpha=0.7)
        for index, bar in zip(indices, bars):
            min_val, max_val = IDEAL_RANGES[index]
            plt.axhspan(min_val, max_val, color='green', alpha=0.1)
        ...
        for bar, value in zip(bars, values):
            yval = bar.get_height()
            status = 'Good' if colors[bars.index(bar)] == 'green' else 'High' if value > IDEAL_RANGES[indices[bars.index(bar)]][1] else 'Low'
...
def make_soil_properties_chart(ph, sal, oc, cec, lst):
    try:
        ...
        for prop, bar in zip(["pH", "Salinity", "Organic Carbon", "CEC", "LST"], bars):
            if prop == "Salinity":
                plt.axhspan(0, IDEAL_RANGES[prop][1], color='green', alpha=0.1)
            else:
...
def generate_report(params, location, date_range):
    try:
        ...
        if genai and API_KEY != "AIzaSyAWA9Kqh2FRtBmxRZmNlZ7pcfasG5RJmR8":
            try:
        ...
        if genai_configured:
            try:
                prompt = f"""
                Generate an executive summary for a soil health report in bullet-point format with:
                - Strengths: Parameters within ideal ranges.
                - Issues: Parameters outside ideal ranges.
                - Recommendations: Practical actions based on issues.
                Data:
                - Location: {location}
                - Date Range: {date_range}
                - Soil Health Score: {score:.1f}% ({rating})
                - Parameters: pH={params['pH'] or 'N/A'}, Salinity={params['Salinity'] or 'N/A'}, Organic Carbon={params['Organic Carbon']*100 if params['Organic Carbon'] else 'N/A'}%, CEC={params['CEC'] or 'N/A'}, Soil Texture={TEXTURE_CLASSES.get(params['Soil Texture'], 'N/A')}, NDVI={params['NDVI'] or 'N/A'}, EVI={params['EVI'] or 'N/A'}, FVC={params['FVC'] or 'N/A'}, NDWI={params['NDWI'] or 'N/A'}, N={params['Nitrogen'] or 'N/A'}, P={params['Phosphorus'] or 'N/A'}, K={params['Potassium'] or 'N/A'}
                Ideal Ranges: {', '.join([f'{k}: {v[0]}-{v[1]}' if isinstance(v, tuple) and len(v) == 2 and v[0] is not None and v[1] is not None else f'{k}: <= {v[1]}' if v[0] is None else f'{k}: {v}' for k, v in IDEAL_RANGES.items()])}
                """
            ...
            executive_summary = "Summary unavailable; Gemini API not configured or invalid key."
        ...
        elements.append(Paragraph(f"<b>Location:</b> {location}", bold_body))
        elements.append(Paragraph(f"<b>Date Range:</b> {date_range}", bold_body))
        elements.append(Paragraph(f"<b>Generated on:</b> {datetime.now():%B %d, %Y %H:%M}", bold_body))
        ...
        elements.append(Paragraph("4. Soil Health Rating", h2))
...
if map_data and "last_active_drawing" in map_data:
    try:
        ...
        if sel and "geometry" in sel and "coordinates" in sel["geometry"]:
            ...
            st.error("Invalid region selected. Please draw a valid polygon.")
    except Exception as e:
