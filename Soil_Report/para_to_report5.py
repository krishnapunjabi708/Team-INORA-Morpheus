...
def get_lst(region,start,end):
    """
    Fetch mean Land Surface Temperature (°C) over the previous calendar month
    for the given EE geometry `region`.
    """
    # Define window: one month ago → today
    end = date.today()
...
def generate_report(params, location, date_range):
    try:
        ...
        if genai and API_KEY != "YOUR_VALID_GEMINI_API_KEY":
            try:
                genai.configure(api_key=API_KEY)
                model = genai.GenerativeModel(MODEL)
                # Test prompt to verify API
        ...
        if genai_configured:
            ...
            recommendations = "Recommendations unavailable; Gemini API not configured or invalid key."
        ...
        try:
            ...
            logging.warning(f"Could not load logo image {LOGO_PATH}: {e}")
