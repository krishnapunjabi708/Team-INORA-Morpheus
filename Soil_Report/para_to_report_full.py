...
# Configuration
API_KEY = "AIzaSyAWA9Kqh2FRtBmxRZmNlZ7pcfasG5RJmR8"  # Provided Gemini API key
MODEL = "models/gemini-1.5-flash"  # Provided model name
LOGO_PATH = "LOGO.jpg"  # Provided logo path
    ...
    try:
        ...
        if genai:
            try:
                ...
                if response and hasattr(response, 'text') and response.text:
                    ...
                    logging.error("Gemini API test prompt returned no valid response.")
            except Exception as e:
        ...
        if genai_configured:
            try:
                ...
                executive_summary = response.text if response and hasattr(response, 'text') and response.text else "Summary unavailable."
                ...
                recommendations = response.text if response and hasattr(response, 'text') and response.text else "Recommendations unavailable."
            except Exception as e:
                logging.error(f"Gemini API error during content generation: {str(e)}")
