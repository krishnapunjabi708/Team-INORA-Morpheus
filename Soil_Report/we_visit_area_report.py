...
if start_date > end_date:
    ...
    st.stop()
# Define the region using provided coordinates
coords = [
...
# Map (still displayed for visualization, centered on the provided coordinates)
m = folium.Map(location=[18.471, 73.988], zoom_start=15)  # Centered on approximate center of coordinates
Draw(export=True).add_to(m)
folium.TileLayer("https://mt1.google.com/vt/lyrs=s&x={x}&y={y}&z={z}", attr="Google").add_to(m)
folium.Marker([18.471, 73.988], popup="Center").add_to(m)
# Add the polygon to the map for visualization
folium.Polygon(
    locations=[[lat, lon] for lon, lat in coords],  # Convert to [lat, lon] for Folium
    color="blue",
    weight=2,
    fill=True,
    fill_opacity=0.2
...
if region:
    ...
    if st.button("Generate Soil Report"):
        with st.spinner("Generating report…"):
            location = f"Lat: 18.471, Lon: 73.988"  # Approximate center for report
    ...
    st.error("Region definition failed.")